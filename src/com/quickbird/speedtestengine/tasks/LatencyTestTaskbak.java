		...
		protected TestParameters doInBackground(URL[] paramArrayOfURL) {
			...
			for (int count = 0; count <= mSampleCount + 1; count++) {
				try {
					...
					LatencyTestTaskbak.this.setError(SpeedTestError.DEVICE_NOT_ONLINE);
					LatencyTestTaskbak.this.failed(SpeedTestError.DEVICE_NOT_ONLINE);
					setCompleted(true);
				} catch (Exception localException1) {
					n = 0;
					DebugUtil.e("LatencyTestTask"+ localException1.getMessage());
					LatencyTestTaskbak.this.setError(SpeedTestError.TEST_RUN);
