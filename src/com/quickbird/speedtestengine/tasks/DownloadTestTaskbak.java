...
public class DownloadTestTaskbak extends SpeedTestTask {
    protected static final String LOGTAG = "DownloadTestTask";
    protected static final double MAXSIZE = 2022400.0D;

    public DownloadTestTaskbak(TestTaskCallbacks paramTestTaskCallbacks, int paramInt) {
    ...
    protected class DownloadSpeedTask extends SpeedTestTask.SpeedTask {
        ...
        private void processDownload(URL paramURL, TestParametersTransfer paramTestParametersTransfer) {
            ...
            try {
                ...
                DownloadTestTaskbak.this.setError(SpeedTestError.TEST_RUN_IO);
            } catch (Exception localException) {
                DebugUtil.e("DownloadTestTask", "Download test failed" + localException);
                DownloadTestTaskbak.this.setError(SpeedTestError.TEST_RUN);
            }
        }
        ...
        protected TestParameters doInBackground(URL[] paramArrayOfURL) {
            ...
            try {
                DownloadTestTaskbak.this.setError(SpeedTestError.None);
                setStartTime(SystemClock.uptimeMillis());
                processDownload(paramArrayOfURL[0], localTestParametersTransfer);
                if (DownloadTestTaskbak.this.getError() == SpeedTestError.None) {
                    localTestParametersTransfer.setSuccess(true);
                    DownloadTestTaskbak.this.success();
                    setCompleted(true);
                    return localTestParametersTransfer;
                }else{
                    localTestParametersTransfer.setSuccess(false);
                    DownloadTestTaskbak.this.failed(DownloadTestTaskbak.this.getError());
                }
            } catch (Exception localException) {
                while (true) {
